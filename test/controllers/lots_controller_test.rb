...
class LotsControllerTest < ActionController::TestCase
  ...
  test "should create lot" do
    assert_difference('Lot.count') do
      post :create, lot: { category: @lot.category, description: @lot.description, status: @lot.status, title: @lot.title }
    end
  ...
  test "should update lot" do
    patch :update, id: @lot, lot: { category: @lot.category, description: @lot.description, status: @lot.status, title: @lot.title }
    assert_redirected_to lot_path(assigns(:lot))
  end
